...
- Top-level function (with variable names):

    When you write variable names on the top-level type, lambdas are added implicity. For example:
...
- Anonymous function (with variable types):
    
    You can avoid top-level definitions with lambda expressions:
...
- Anonymous function (with full type):
    ...
    Optionally, you can annotate the full type rather than the type of each variable:
...
- Anonymous function (untyped)

    Or you can avoid types entirely. This won't type-check, but Formality won't stop you from running it.
