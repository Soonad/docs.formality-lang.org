...
The type above allows you to create a list of N nums:
...
Self Types allow a type to access *its own value*. This has many uses. Suppose that you wanted to create a pair of identical nums:
...
same_nums_0 : SameNums 
  new(~sameNums) [0, 0, refl(~Num, ~%0)]

same_nums_1 : SameNums 
  new(~sameNums) [1, 1, refl(~Num, ~%1)]

same_nums_2 : SameNums 
  new(~sameNums) [2, 2, refl(~Num, ~%2)]
```

Notice how the `SameNums` type has access to values of its terms. So, when we instantiate `same_nums_0`, the `self` variable on the type is replaced by `[0, 0, refl(~Num, ~%0)]`, which becomes `[:Num, :Num, Equal(Nat, %0, %0)]`, allowing us to write the last element a `refl(~Num, ~%0)`. Of course, in this case, this effect could be achieved with dependent pairs:

```haskell 
SameNumsB : Type
  [x : Num, y : Num, Equal(Num, %x, %y)]

same_nums_b_0 : SameNumsB
  [0, 0, refl(~Num, ~%0)]

same_nums_b_1 : SameNumsB
  [1, 1, refl(~Num, ~%1)]

same_nums_b_2 : SameNumsB
  [2, 2, refl(~Num, ~%2)]
```
