...
The `+ note e : x is true` syntax reminds Formality that `x is true`, making it specialize `x` on each branch, giving you an `e : true is true` on the `true` branch, and an `e : false is true` on the `false` branch. Since `false is true` is absurd, we know the `false` branch is unreachable and, thus, can fill it by exploiting the `absurd` function above, without writing any number.
