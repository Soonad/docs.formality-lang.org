...
Formality includes a powerful datatype system. A new datatype can be defined with the `T` syntax, which is similar to 's `data`, and creates global definitions for its type and constructors. To pattern-match against a value of a datatype, you must use `case/T`.
...
Notice the `case` keyword before `suit`. This tells Formality to match `suit` directly, giving `print_suit` a -like syntax. This feature is actually quite powerful, as it will perform some handy resource management and "motive-filling" under the hoods, allowing you to write much smaller programs/proofs. Use it when possible!
