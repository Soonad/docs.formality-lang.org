...
Differently from, say, Agda, Formality doesn't have a dependent pattern-matching syntax that will automatically collapse unreachable cases with a `()`. Instead, we just have case expressions, which desugar to plain dependent eliminations. You must always provide all cases. When you know a case is unreachable, you must use the motive to return anything (say, `unit`) on that case.
...
// If len > 0, demand `Vector(T, pred(len))`, otherwise, demand anything (Unit)
vtail.motive : {T : Type, len : Nat, self : Vector(T, len)} -> Type
  case/Nat len
  | succ => Vector(T, pred)
  | zero => Unit
  : Type
...
vtail : {~T : Type, ~len : Nat, vector : Vector(T, succ(len))} -> Vector(T, len)
  ...
  | vcons => tail
  | vnil  => unit
  : vtail.motive(T, len, self)
```

Notice that, on the `vnil` case, Formality will expect you to provide a `vtail.motive(T, zero, vnil)`, which normalizes to `Unit`, so we can provide whatever (i.e., `unit`). Yet, when computing the return-type of the case-expression, it uses the specific matched value, i.e., `vtail.motive(T, succ(len), vector)`, which never reaches the vnil case and, thus, normalizes to `Vector(T, len)`.
