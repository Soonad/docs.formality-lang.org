...
When using `case` arguments, `move`s are added automatically:
...
The `+ note e : x == true` syntax reminds Formality that `x == true`, making it specialize `x` on each branch, giving you an `e : true == true` on the `true` branch, and an `e : false == true` on the `false` branch. Since `false == true` is absurd, we know the `false` branch is unreachable and, thus, can fill it by exploiting the `absurd` function above, without writing any number.
...
**TODO**: write a brief explanation on how Self encodings work (although I think it should be self-explanatory from this example!).
