...
This tells you that, on the location of the hole you should have a `Bool`. Formality will accept a hole as true, allowing you to move on to other parts of your program before returning to it.

Another handy feature is `log(x)`. When running a program, it prints the normal form of `x`, similarly to JavaScript's `console.log` and Python's `print`, but for anything (not only strings). When type-checking a program, it tells you the normal-form and the type of `x`. This is useful when you want to know what type an expression would have inside certain context. For example:
...
Note that holes assume that the inferred type at given location is correct. If that location has no inferred type, Formality will output a type-error and stop type-checking, preventing you from working on other parts of your program. To solve that, just prepend an explicit annotation with `:`, as in `: MyType ?`.
