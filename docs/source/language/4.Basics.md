...
Since this increases the power of the language considerably, in order not to cause logical inconsistencies, and to allow efficient reductions, boxes are limited by the "stratification condition". It enforces that the number of `#`s surrounding a term must never change during reduction. This will be explained in more details later on.
